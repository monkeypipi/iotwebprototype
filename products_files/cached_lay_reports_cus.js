...
;(function($,win,doc,UMENG,undefined){
    var BLACKLIST = ['index', 'duration','frequency','depth','interval','devices_devices','devices_resolutions','devices_versions','network_access','network_carriers','location_cities','location_countries', 'statschannel', 'channel_retention', 'funnels', 'stage_stats', 'payment_trend', 'consume_stats', 'ltv_ltv', 'channels_custom', 'currency'];
    UMENG = win.UMENG || {};
    ...
    function natureLocalDate(){
        ...
        if(flag > -1 && expire == localStorage['localCacheDateExpireTime_natureCacheDate'] && UMENG.plugin.userCustomDate.localCache){
            if(localStorage['natureCacheDate']){
                ...
                $('.select_value').attr('timeunit',timeUnit[win.global.pickedDays]);
            }
        }
    };
    ...
    if( reports === 'true' )
    {
      $.ajax({
        ...
        success: function(res){
          if( 'android' === platform && 'module' === sdk_type ){
            v = res[0][0];
            d = res[0][1];
          } else {
            v = res[0];
	...
	return this.each(function() {
		...
		function drawLinks() {
            ...
            if(opts.showCustomPage){
                ...
                panel.prepend('<div class="fl" style="margin-left: 5px;margin-top: 10px;">'+I18n.t("components.pager.perpage")+'：<div class="per-page js-select"><div class="selected" val="'+ perPage +'">'+ perPage +'<b class="select-corner"></b></div><ul class="options hideWhenClickBody"><li>30</li><li>60</li><li>90</li><li>120</li></ul></div></div>');
    ...
    Plugin.prototype = {
        init: function () {
            var _ = this,
                ...
                $guide = $('<div style="width:'+_.settings.width+'px;" class="arrow-box arrow-box-' + _.settings.corner + '"><p style="width:'+ (_.settings.width-33) +'px;">' + _.settings.text + '</p><span class="arrow-box-close">x</span></div>');
...
;(function($,undefined){
    $.widget('umeng.renderChart',{
               ...
               $.ajax({
                   ...
                   success: function(results){
                       if(results.result != 'success'){
                           ...
                           that._renderSeries(results.dates, results.stats, results.summary);
                       }
                   }
               })
           }
       ...
       _renderSeries: function(categories, serie, summary){
           ...
           $.each(serie, function(i,serie){
               if(serie.data.length == 1){
                   series[i] = $.extend(true,{visible:true}, {marker:{enabled: true}},serie );
               }else{
                   series[i] = $.extend({visible:true}, serie);
               }
           ...
           this._writeCache(currentChartOptions);
